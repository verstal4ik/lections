...
def main(timbers, group = [18, 22, 25, 35]):
	...
	regsort - список зарегистрированных сортов, которые формирует окорочный станок
	В regsort важен порядок, чем ближе к началу, тем больше приоритет для выбора
	"""

	Diametr_KM = 50 
	d_min = group[0]
	regsort = ["Металл", "C", "Cx", "D", "Dc", "KM", "AB"]
	header_def = ["Диаметр <18" , "D (кривизна)", "D (комель > 50,см", "Металл", "L (длина < 5.7 м)", "C (сучки, пасынки, заком-ть)", "Итого, брак"]
	sort_matching = {"Металл": 3,
					"C": 5,
					"Cx":,
					"D":,
					 "Dc": ,  }
	...
	n6 = n57 = ndef = 0
	while i < N: 
		diametr = (min(timbers[i][4], timbers[i+1][4])) 
		...
		if sort == "AB" and length == 6 and diametr > d_min:
			...
			ndef += 1
			add_to_table_def(diametr = diametr, sort = sort, table = timbers_def, regsort = regsort, header_def = header_def )
			
		i += 2

	return timbers6m, timbers57m, timbers_def, timbers1C, timbers_data
...
def assign_sort(sort1, sort2, regsort, length1=3.05, length2=3.05):
	...
	sort = "Неопределен"
	#FIXME
	#Кажеnся можно изящнее проверить вхождение
...
def check_length(l1:str, l2:str, sort = None):
...
def add_to_table_def(diametr, sort, table, regsort, header_def):
	"""Функция заполняет таблицу с теми сортами которые попали в брак.
	Зачет объема будем делать по 6 метрам, кривые может по 5.7 считать, которые 5.7?

	"""

	#Как сопоставить сорт брака и название в таблице ?
	#print (diametr, sort, regsort, header_def, "\n", sep="\n")
	#
	print (table),
	return -1
